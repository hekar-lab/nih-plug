...
use crate::{
    futures, Application, core::Element, Command, IcedEditor, ParameterUpdate,
...
impl<E: IcedEditor> Application for IcedEditorWrapperApplication<E> {
    ...
    fn subscription(
        ...
        let mut editor_window_subs = WindowSubs {
            on_frame: match &window_subs.on_frame {
                Some(func) => match func() {
                    Some(Message::EditorMessage(msg)) => {
        ...
        let subscription = Subscription::<Self::Message>::batch([
        ...
        if let Some(func) = editor_window_subs.on_frame {
            if let Some(msg) = func() {
                window_subs.on_frame = Some(Arc::new(move || {
                    Some(Message::EditorMessage(msg.clone()))
                }));
            }
        }

        if let Some(func) = editor_window_subs.on_window_will_close {
            if let Some(msg) = func() {
